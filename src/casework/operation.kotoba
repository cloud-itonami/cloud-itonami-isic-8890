(ns casework.operation
  "OperationActor -- one social-work/casework operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (CaseworkOps-LLM) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Social Services Governor
  ...
  One graph run = one social-work/casework operation (intake -> advise
  ...
  decision to a human operator (the caseworker). The approver resumes
  with `{:approval {:status :approved}}` (or :rejected). `:eligibility/
  finalize`/`:referral/finalize` ALWAYS reach this node when the
  governor is clean -- see `casework.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [casework.caseworkopsllm :as caseworkopsllm]
...
(defn build
  ...
  `casework.store/Store`).
  opts:
    :advisor      -- a `casework.caseworkopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (caseworkopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; CaseworkOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (caseworkopsllm/-advise advisor store request)]
            {:proposal p :audit [(caseworkopsllm/trace request p)]})))

      ;; Social Services Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
